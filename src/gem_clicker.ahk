...
F6::
    ...
    if (Toggle) {
        ...
        if (!Adb_VerifyHarness(lockedHwnd)) {
...
Adb_VerifyHarness(hwnd) {
    global adbFramePath, adbFrameReady, adbVerified, adbWmWidth, adbWmHeight
    adbVerified := false
    adbFrameReady := false
    if (!hwnd) {
    ...
    arClient := cw / ch
    arAdb := fw / fh
    if (Abs(arClient - arAdb) > 0.05) {
        Adb_Log("verify: aspect ratio mismatch (client=" arClient " adb=" arAdb ")")
        Gdip_DisposeImage(pBmp)
        return false
    }
    adbHeader := DetectHeaderH_Bitmap(pBmp, fw, fh)
    Gdip_DisposeImage(pBmp)
    clientHeader := DetectHeaderH(hwnd, cw, ch)
    if (ch > 0 && fh > 0) {
        headerDelta := Abs((clientHeader / ch) - (adbHeader / fh))
        if (headerDelta > 0.08) {
            Adb_Log("verify: header ratio mismatch (client=" clientHeader "/" ch " adb=" adbHeader "/" fh ")")
            return false
        }
    }
    adbVerified := true
